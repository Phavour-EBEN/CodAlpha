...
if __name__ == '__main__':
    ...
    while(1):
        ...
        for batch_id, x0, y0, x1, y1, cls_id, score in outputs:
            ...
            if(score > args.tresh):
                # Draw Bounding box
                cv.rectangle(image, box[:2], box[2:], COLORS[cls_id], 2)
                # Draw class name and confidence score
                cv.putText(image, name, (box[0], box[1] - 2), cv.FONT_HERSHEY_SIMPLEX, 0.55, COLORS[cls_id], thickness=2)

        grabbed = False
        cv.imshow("Detected",image)
        if cv.waitKey(key) ==  ord('q'):
            break        